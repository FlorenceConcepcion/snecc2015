...
def showcurve(sn, hml, source_name):
    ...
    peaktime = modellimit(z0, source_name)
    for row in hml:
        datestamp = row[0]
...
b_source_names = ['nugent-sn1bc', 's11-2005hl', 's11-2005hm', 'snana-2004gv', 'snana-2006ep', 'snana-2007y', 'snana-2004ib', 'snana-2005hm', 'snana-2007nc']#'snana-2006jo']#, 
c_source_names = ['nugent-sn1bc','snana-04d1la', 'snana-04d4jv', 'snana-2004fe', 'snana-2004gq', 'snana-sdss004012', 'snana-2006fo', 'snana-sdss014475', 'snana-2006lc', 's11-2006fo']
...
for sn in completesnlist[:]:
    ...
    for source_name in source_name_list:
        ...
        try:
            ...
            #print y
            if y != None:
                redchilist.append([y[0], y, source_name])
        except: 
            pass

    distancefrom = [abs(i[0]-1) for i in redchilist]
    if len(distancefrom) ==0:
        pass
    else:
    
        
        bestdisvalue = np.min(distancefrom)
        bestredchivalue = redchilist[distancefrom.index(bestdisvalue)]
        y = bestredchivalue[1]
        redshiftlimit = 0.1
        redchilimit = 30
        if y[2] < redshiftlimit and y[0] < redchilimit:
            ...
            source=sncosmo.get_source(bestredchivalue[2]) 
            model=sncosmo.Model(source=source)
           ...
           # plt.gca().invert_xaxis()
            ...
            '''
            print sn, 'abs peak mag', absmagvalue, 'model', source_name
...
plt.title("Peak Absolute Magnitude Histogram of TypeIb and TypeIc")
...
plt.title("Reduced Chi Squared Distribution of TypeIb/c")
...
plt.title("Redshift Distribution of TypeIb/c")
plt.xlabel("Redshift")
plt.ylabel("Frequency")
plt.legend()
