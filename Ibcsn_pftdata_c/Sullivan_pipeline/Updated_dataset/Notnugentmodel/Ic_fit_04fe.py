...
def modellimit(z0):
    source=sncosmo.get_source('snana-2004fe') 
...
snblist = ['09dfk', '09dha', '09fsr', '09ut', '10acff', '10acgq', '10eqi', '10fbv', '10feq', '10inj', '10kui', '10qif', '10vnv', '10xjr', '11bli', '11gcj', '11ilr', '11izq', '11kaa', '11mwk', '11qcj', '11qiq', '12cde', '12eaw', '12lpo', '12mfx']
''' Type Ib/c are in both snblist and snclist
'''
...
plt.plot(x, mlab.normpdf(x,mean,sigma)*scale, label = 'Best fit, mean: ' +str(round(mean, 3))+' sigma: ' + str(round(sigma, 3)),color = 'green')
print 'Ic mean:', mean, 'sigma:', sigma

plt.title("Peak Absolute Magnitude Histogram of TypeIb and TypeIc using the snana-2004fe model")
...
plt.plot(x, mlab.normpdf(x,mean,sigma)*scale, label =  'mean: ' +str(round(mean, 3))+' sigma: ' + str(round(sigma, 3)), color = 'green')
...
plt.title("Reduced Chi Squared Distribution of TypeIb/c using the snana-2004fe model")
...
plt.title("Redshift Distribution of TypeIb/c using the snana-2004fe model")
plt.xlabel("Redshift")
plt.ylabel("Frequency")
plt.legend()
