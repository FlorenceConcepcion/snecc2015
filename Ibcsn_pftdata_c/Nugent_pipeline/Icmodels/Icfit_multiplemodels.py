...
x=open('/home/fcm1g13/Documents/Supernova/Ibcsn_pftdata_c/Nugent_pipeline/SNlist.txt')  
...
datadir = '/home/fcm1g13/Documents/Supernova/Nugent_pipeline/Ibcsn_pftdata_c/'
...
source_names = ['snana-04d1la', 'snana-04d4jv', 'snana-2004fe', 'snana-2004gq', 'snana-sdss004012', 'snana-2006fo', 'snana-sdss014475', 'snana-2006lc', 's11-2006fo']
source_name = 'snana-2004gq'
source=sncosmo.get_source(source_name) 
model=sncosmo.Model(source=source)
jo=jp=1
            ...
            if types[sn[:-7]][1] !='b':
                ...
                if len(hml) >4:
                    ...
                    if y != None:
                        ...
                        timearray = np.arange(y[0][1]-20,y[0][1]+50,1)
...
'''                        
    ...
    listofchimodel.append(listofchisource)
print listofchimodel
print listofchimodel.index(np.max(listofchimodel))  
print source_names[listofchimodel.index(np.max(listofchimodel))]            
...
'''     
plt.subplot(211)  
peakabsmagvalue = [peakabsmagvaluec]#,peakabsmagvaluec] 
plt.hist(peakabsmagvalue, bins = redchilimit/2, label = [str(ic) + ' Ic datapoints' ], color = ['blue']) #, stacked=True)
#plt.hist(peakabsmagvaluec, bins = redchilimit/0.5, label = str(ic) + ' datapoints', color = 'green',stacked=True)
plt.title("Peak Absolute Magnitude Histogram of TypeIc using " + source_name)
plt.xlabel("Absolute Magnitude")
plt.ylabel("Frequency")
plt.legend()

plt.subplot(212)            
redchi = [redchic]#,redchic]
plt.hist(redchi, bins = redchilimit/2, label = [str(ic) + ' Ic datapoints'], color = ['blue']) #, stacked=True)
#plt.hist(redchic, bins = redchilimit/0.5, label = str(ic) + ' datapoints', color = 'green')

plt.title("Reduced Chi Squared Distribution of TypeIc using " + source_name)
